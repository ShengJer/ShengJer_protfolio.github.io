...
def parse_academic_project(academic_project_dir):
    """Parse academic project items from the _academic_project directory."""
    academic_project = []
    
    if not os.path.exists(academic_project_dir):
        return academic_project
    
    for academic_project_file in sorted(glob.glob(os.path.join(academic_project_dir, "*.md"))):
        with open(academic_project_file, 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        if front_matter_match:
            ...
            # Extract academic project details
            academic_project_entry = {
                "name": front_matter.get('title', ''),
                "category": front_matter.get('collection', 'academic_project'),
                "date": front_matter.get('date', ''),
                "url": front_matter.get('permalink', ''),
                "description": front_matter.get('excerpt', '')
            }
            
            academic_project.append(academic_project_entry)
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    cv_json["academic_project"] = parse_academic_project(os.path.join(repo_root, "_academic_project"))
